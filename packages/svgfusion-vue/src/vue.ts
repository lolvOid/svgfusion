...
${children}
  ...
  private generateSvgAttributes(ast: {
    ...
    // Ensure we have xmlns if not present
    if (!allAttributes.xmlns) {
      allAttributes.xmlns = 'http://www.w3.org/2000/svg';
    }

    // Skip width/height if viewBox is present (they'll be handled by computed props)
    const hasViewBox = allAttributes.viewBox;

    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    Object.entries(allAttributes).forEach(([key, value]) => {
      if (value !== undefined && value !== null) {
        // Skip width/height when viewBox is present (handled by :width/:height bindings)
        if (hasViewBox && (key === 'width' || key === 'height')) {
          return;
        }

        // Handle special attributes that need Vue template formatting
        if (
          key === 'aria-labelledby' &&
          typeof value === 'string' &&
          value.includes('{')
        ) {
          // Convert template strings like '{titleId} {descId}' to Vue expressions
