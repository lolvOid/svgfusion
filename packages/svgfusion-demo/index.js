...
class DemoRunner {
  constructor() {
    this.demoDir = process.cwd(); // Changed from path.join(process.cwd(), 'demo')
    this.reactDemoDir = path.join(this.demoDir, 'demo-react');
    this.vueDemoDir = path.join(this.demoDir, 'demo-vue');
  }
  ...
  async installDependencies() {
    if (this.directoryExists(this.reactDemoDir)) {
      ...
      await this.runCommand('pnpm install', this.reactDemoDir);
    }

    if (this.directoryExists(this.vueDemoDir)) {
      console.log('\n📦 Installing Vue demo dependencies...');
      await this.runCommand('pnpm install', this.vueDemoDir);
    }
  }
