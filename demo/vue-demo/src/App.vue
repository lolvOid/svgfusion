  ...
  <div class="min-h-screen bg-gray-50">
    <div class="max-w-6xl mx-auto px-4 py-8">
      <header class="text-center mb-8">
        <h1 class="text-4xl font-bold text-green-600 mb-2">🎨 SVGFusion Vue Demo</h1>
      ...
      <main class="space-y-8">
        <section class="bg-white p-6 rounded-lg shadow-sm">
          <div class="flex flex-wrap gap-6 justify-center">
            <div class="flex flex-col items-center space-y-2">
              <label class="font-semibold text-gray-700">Size: {{ size }}px</label>
              <input
                v-model="size"
                type="range"
                min="16"
        ...
        <section class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          <div v-for="icon in icons" :key="icon.name" class="bg-white p-6 rounded-lg shadow-sm border hover:-translate-y-1 hover:shadow-md transition-all duration-200">
            <div class="flex flex-col items-center space-y-4">
              <component 
                ...
                :title="icon.name"
              />
              <span class="text-sm text-gray-600 font-medium">{{ icon.name }}</span>
            </div>
          </div>
        </section>

        <section class="bg-white p-6 rounded-lg shadow-sm">
          <h2 class="text-2xl font-semibold text-gray-800 mb-6 text-center">Size Variations</h2>
          <div class="flex gap-8 justify-center items-end flex-wrap">
            <div v-for="iconSize in [16, 24, 32, 48, 64]" :key="iconSize" class="flex flex-col items-center space-y-2">
      ...
      <footer class="mt-12 pt-8 border-t border-gray-200 text-center">
        <p class="text-sm text-gray-500">Generated by SVGFusion • Composition API • Script Setup</p>
      </footer>
    </div>
  </div>
</template>
